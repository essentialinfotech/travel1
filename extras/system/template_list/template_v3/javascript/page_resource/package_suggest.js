var flight_adult_child_max_count = 6;
var cache = {};
    ...
    $('#flight-form-submit').on('click', function(e) {
    	...
    	if(trip_type == 'oneway' || trip_type == 'circle') {//Oneway/roundWay
	        ...
	        if(!_from_loc && !_to_loc){
	            show_alert_content('From location and To location cannot be empty.', '#flight-alert-box');
	            e.preventDefault();
	            return ''
	        }
	        if (_from_loc == _to_loc) {
    ...
    $(".fromflight, .departflight").catcomplete({
        ...
        select: function(event, ui) {
            ...
            if (this.id == 'to') {
                ...
                $('.to_airport').html(ui.item.airport_name);
            } else if (this.id == 'from') {
                $('.from_airport').html('');
                from_airport = ui.item.value
                $('.from_airport').html(ui.item.airport_name);
            }
...
function manage_infant_count(pax_type) 
{
    ...
    if (pax_type == 'adult') {//Adult
    		...
    		if (temp_adult_child_count >= flight_adult_child_max_count) {
                ...
                _content = '<small>Max 6 Passenger(Adult+Child) Allowed</small>'
            } else {
